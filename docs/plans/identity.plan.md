...
- Endpoint `RegisterUser` (Wolverine.Http) — tự đăng ký, thay cho việc seed tay user test.
- Endpoint `Login` (Wolverine.Http, JSON) — xác thực + set auth cookie cho luồng `/connect/authorize`.
...
|---|---|---|---|---|
| POST | `/account/register` | email, password | 201 + userId | Public, `UserManager.CreateAsync`, **gọi qua Gateway** |
...
**Endpoint nghiệp vụ**
- [ ] `Api/RegisterUserEndpoint.cs` — `[WolverinePost("/account/register")]`.
- [ ] `Api/LoginEndpoint.cs` — `[WolverinePost("/account/login")]`.
...
- **`CustomerId`** để nullable, chưa liên kết thật tới Customer service (chưa tồn tại).
